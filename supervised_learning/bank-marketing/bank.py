...
"""A driver module for running five machine learning algorithms on a bank
marketing data set.
...
# We must convert the possibly classes into numbers, so that NumPy can take it
# in natively.
def create_mapper(l):
    return {l[n] : n for n in xrange(len(l))}

job = create_mapper(['\"admin.\"', '\"unknown\"', '\"unemployed\"',
                     '\"management\"', '\"housemaid\"', '\"entrepreneur\"',
                     '\"student\"', '\"blue-collar\"', '\"self-employed\"',
                     '\"retired\"', '\"technician\"', '\"services\"'])

marital = create_mapper(['\"married\"', '\"divorced\"', '\"single\"'])

education = create_mapper(['\"unknown\"', '\"secondary\"', '\"primary\"',
                           '\"tertiary\"'])

default = create_mapper(['\"yes\"', '\"no\"'])

housing = create_mapper(['\"yes\"', '\"no\"'])

loan = create_mapper(['\"yes\"', '\"no\"'])

contact = create_mapper(['\"unknown\"', '\"telephone\"', '\"cellular\"'])

month = create_mapper(['\"jan\"', '\"feb\"', '\"mar\"', '\"apr\"', '\"may\"',
                       '\"jun\"', '\"jul\"', '\"aug\"', '\"sep\"', '\"oct\"',
                       '\"nov\"', '\"dec\"'])

poutcome = create_mapper(['\"unknown\"', '\"other\"', '\"failure\"',
                          '\"success\"'])

y = create_mapper(['\"yes\"', '\"no\"'])

converters = {
    1: lambda x: job[x],
    2: lambda x: marital[x],
    3: lambda x: education[x],
    4: lambda x: default[x],
    6: lambda x: housing[x],
    7: lambda x: loan[x],
    8: lambda x: contact[x],
    10: lambda x: month[x],
    15: lambda x: poutcome[x],
    16: lambda x: y[x]
}

in_file = "bank-full.csv"

def load_data(filename):
    with open(filename) as data:
        # Remove lines with unknown data (denoted by "?").
        data_list = [line for line in data if "?" not in line]
...
def initialize():
    ...
    tx, ty = np.hsplit(tr, [16])
    rx, ry = np.hsplit(te, [16])
    ty = ty.flatten()
    ry = ry.flatten()
    return tx, ty, rx, ry
